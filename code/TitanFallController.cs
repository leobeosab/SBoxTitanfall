...
namespace dotasmsplayground
{
	class TitanFallController : BasePlayerController
	{
		...
		public override void Simulate()
		{
			...
			if (Input.Pressed( InputButton.Jump ) && !isJumping)
			{
				isJumping = true;
				CheckJumpButton();
			}
		...
		public virtual void CheckJumpButton()
		{
			...
			Log.Info(Sandbox.Input.Pressed( InputButton.Jump ));

			if ( GroundEntity == null && false )
			{
				return;
			}
			else if (GroundEntity == null && !hasDoubleJumped)
			{
				Log.Info( "Double Jumping" );
			...
			float startz = GroundEntity != null ? Velocity.z : 1f;
			float jumpVelocity = JumpStrength * (GroundEntity == null ? DoubleJumpModifier : 1.0f);

			Velocity = Velocity.WithZ( startz + jumpVelocity);
			Log.Info( JumpStrength );
